(function () {
  ...
  function updateUI() {
    ...
    if (input && total) {
      ...
      if (document.activeElement !== input) {
        input.value = sanitizePageInput(pageStr || '1');
      }
    }
  ...
  function init() {
    ...
    window.addEventListener('r3d-pagechange', function (e) {
      if (e.detail && e.detail.page) {
        var input = document.getElementById('page-input');
        if (input && document.activeElement !== input) {
          input.value = sanitizePageInput(e.detail.page);
        }
      }
      updateUI();
    });
    ...
    var ready = setInterval(function () {
      if (flipbook.Book) {
        clearInterval(ready);
        if (flipbook.updateCurrentPage) flipbook.updateCurrentPage();
        updateUI();
      }
    }, 200);
    ...
    if (input) {
      ...
      input.addEventListener('input', function () {
        input.value = sanitizePageInput(input.value);
      });
    }
