const View = {
            ...
            <section class="admin-panel">
                <div class="admin-toolbar">
                    <div class="admin-toolbar-left">
                        <select id="offer-bulk-action" class="admin-bulk-select">
                        ...
                        <select id="offer-status-filter" class="admin-filter-select">
                    ...
                    <div class="admin-toolbar-right">
                        <input id="offer-search" class="admin-search-input" type="search" placeholder="Search offers">
    ...
    rowTable(item) {
        const dateText = [item.start_at, item.end_at].filter(Boolean).join(" → ") || item.updated_at || "N/A";
        return `
            <tr>
                ...
                <td>Marketing</td>
                <td>${item.provider_name || item.cta_text || "Offer"}</td>
...
(() => {
    ...
    Api.Offer.GetAll()
        .done((response) => {
            ...
            if (!items.length) {
                $("#offer-table-body").html(`<tr><td colspan="6"><div class="admin-empty-state">Chưa có ưu đãi nào.</div></td></tr>`);
            } else {
                $("#offer-table-body").html(items.map(View.rowTable).join(""));
            }

            AdminDataTable.mount({
                selector: "#offers-table",
                searchInputSelector: "#offer-search",
                statusFilterSelector: "#offer-status-filter",
                statusColumnIndex: 4,
                pageLength: 10,
                order: [[5, "desc"]],
                zeroRecords: "Chưa có ưu đãi phù hợp bộ lọc",
                emptyTable: "Chưa có ưu đãi nào",
            });
        })
        .fail(() => {
            root.innerHTML = `<div class="admin-panel admin-empty-state">Không tải được danh sách ưu đãi.</div>`;
        });
})();
